...
class HFTransformerModel(nn.Module):
    def __init__(self,
        ...
        self.device = device
        self.src_pad = item_num  # align pad id with Train_instance (-1 -> item_num)
    ...
    def forward(self, batch_x, seq_x, batch_y, data_emb=None, orig_y=None, type=0, use_con=False, use_guide=False, guide_feat=None, src_decoder_emb=None, src_y=None):
        ...
        # Project summary token to the feature space used by pretrained item embeddings.
        global_feat = F.normalize(global_feat, dim=-1)
        global_feat = self.fc_proj1(global_feat)

        contra_loss = global_feat.new_zeros(())
        if use_con:
            with torch.no_grad():
                if isinstance(data_emb, torch.Tensor):
                    data_emb_t = data_emb
                else:
                    data_emb_t = torch.as_tensor(data_emb)
                if data_emb_t.device != global_feat.device:
                    data_emb_t = data_emb_t.to(global_feat.device)
                positive_emb = torch.index_select(input=data_emb_t, index=orig_y, dim=0).float()

            # Global Contrastive Task (GCT): positive-only distillation with SmoothL1.
            contra_loss = F.smooth_l1_loss(global_feat, positive_emb, reduction="mean")
        ...
        return z, contra_loss, global_feat #local_feat_
