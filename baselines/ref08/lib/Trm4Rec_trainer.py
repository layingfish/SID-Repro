...
class Trm4Rec:
    def __init__(self,
        ...
        if self.tree.tree_height != 2:
            import math

            min_k = int(math.isqrt(item_num - 1)) + 1
            raise ValueError(
                f"Unsupported tree_height={self.tree.tree_height}. Current implementation assumes 2-level codes. "
                f"Choose k so that {min_k} <= k < {item_num} (ensuring k^2 >= item_num). "
                f"Got k={k}, item_num={item_num}."
            )
        ...
        self.tree.item_to_code[-1] = torch.full((self.tree.item_to_code.size(-1),), end_idx-1, dtype=torch.int64, device="cuda")
