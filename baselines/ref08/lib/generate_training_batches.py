...
class Train_instance:
    ...
    def read_all_instances_files(self,traing_instaces_path,seg_couts,item_num, his_maxtix=None,labels=None):
        his_maxtix_path = his_maxtix
        labels_path = labels
        if his_maxtix_path and labels_path and os.path.isfile(his_maxtix_path) and os.path.isfile(labels_path):
            his_maxtix = torch.load(his_maxtix_path)
        ...
        for i in range(seg_couts):
            ...
            if his_maxtix is not None:
                ...
                labels=part_labels
        assert len(his_maxtix)==len(labels)
        if his_maxtix_path and labels_path:
            os.makedirs(os.path.dirname(his_maxtix_path) or '.', exist_ok=True)
            os.makedirs(os.path.dirname(labels_path) or '.', exist_ok=True)
            torch.save(his_maxtix, his_maxtix_path)
