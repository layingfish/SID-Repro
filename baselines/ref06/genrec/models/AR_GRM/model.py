...
class AR_GRM(AbstractModel):
    ...
    def forward(self, batch: dict, return_loss=True) -> ModelOutput:
        ...
        dec_gt = torch.clamp(batch['decoder_input_ids'].to(device), 0, self.codebook_size - 1)
        ...
        bos = self.bos_embedding.unsqueeze(0).unsqueeze(1).expand(B_dec, 1, -1)
        dec_inp = torch.cat([bos, tok_emb], dim=1)
        ...
        for i, blk in enumerate(self.decoder_blocks):
            ...
            out = blk(
                ...
                cross_attention_mask=enc_key_mask
            )
            x = out['hidden_states']
    ...
    def generate(self, batch, n_return_sequences=10, mode=None):
        ...
        try:
            with torch.no_grad():
                ...
                keep_k = min(beam_num[0], pre_cut[0])
                ...
                for d in range(1, self.n_digit):
                    ...
                    tk_prob, tk_idx = torch.topk(logp, k=pre, dim=-1)
                ...
                sorted_beams = beams.gather(1, sorted_idx_exp)
                ...
                final = torch.stack(final_list, dim=0)
                return final
        finally:
            if was_training:
                self.train()
