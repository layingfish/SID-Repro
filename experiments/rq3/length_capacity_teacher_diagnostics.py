...
def _teacher_forcing_rows(
    *,
    ...
    root = Path(os.environ.get("RECSYS26_ROOT", Path(__file__).resolve().parents[2])).resolve()
    baseline_decoder_root = Path(os.environ.get("BASELINE_DECODER_ROOT", root / "baselines" / "decoder")).resolve()
    common_root = root / "pipeline" / "common"
    tokenizer_root = root / "pipeline" / "tokenizer"
    eval_root = root / "pipeline" / "evaluation"
    for p in (str(baseline_decoder_root), str(common_root), str(tokenizer_root), str(eval_root)):
...
def _parse_args() -> argparse.Namespace:
    ...
    ap.add_argument("--data_dir", type=Path, default=Path(os.environ.get("DATA_ROOT", default_root / "data")))
    ap.add_argument("--eval_dataset", default="amazon23_vg")
    ap.add_argument("--dataset_folder", default=str(default_root / "data" / "decoder"))
    ap.add_argument("--export_domain", default="amazon23_vg_tiger_strict")
    ap.add_argument("--hf_model_path", default=os.environ.get("HF_MODEL_T5_SMALL", str(default_root / "models" / "t5-small")))
