...
CODE_DIR="${LLM_ID_CODE_DIR:-$RECSYS26_ROOT/baselines/ref02}"
...
run_dir="${RECSYS26_RUN_DIR:-$run_root/$(now_ts)}"
mkdir -p "$run_dir"
...
(
  ...
  case "$domain" in
    microlens_50k|microlens_50k_semid_adapt|microlens_100k|amazon23_vg|yelp)
      eval_cmd+=(--warm_only)
      ;;
  esac
