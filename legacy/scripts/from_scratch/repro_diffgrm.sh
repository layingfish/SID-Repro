...
if [[ "$#" -lt 2 ]]; then
  echo "Usage: repro_diffgrm.sh <domain> <profile>" >&2
...
DIFFGRM_CODE_DIR="$RECSYS26_ROOT/third_party_clean/DiffGRM"
SETREC_DATA_ROOT="$RECSYS26_ROOT/datasets/setrec_data"
CACHE_DIR="$RECSYS26_ROOT/datasets/diffgrm_cache_from_scratch/${domain}_k5"
PATCH_FILE="${RECSYS26_DIFFGRM_PATCH:-$RECSYS26_ROOT/patches/from_scratch_20260228/DiffGRM.patch}"

now_ts() { date -u +%Y%m%d_%H%M%S; }

run_dir="$RECSYS26_ROOT/logs/repro_from_scratch/diffgrm/${domain}/$(now_ts)"
mkdir -p "$run_dir"
ln -sfn "$run_dir" "$RECSYS26_ROOT/logs/repro_from_scratch/diffgrm/${domain}/latest"
...
if [[ "$profile" == "smoke" ]]; then
  extra_train_args+=(--eval_start_epoch=1)
fi
...
(
  ...
  echo "[from_scratch][diffgrm] domain=$domain profile=$profile CUDA_VISIBLE_DEVICES=$CUDA_VISIBLE_DEVICES"
  ...
  train_cmd=(
    ...
    --ckpt_dir=$ckpt_dir
    --run_id=diffgrm_setrec_${domain}_${profile}
    --num_proc=1
    --rand_seed=$seed
