  ...
  case "$method" in
    ...
    rpg)
      ...
      python main.py \
        ...
        --cache_dir="$RECSYS26_ROOT/datasets/rpg_cache_paper" \
        --log_dir="$run_dir" \
    ...
    diffgrm)
      ...
      python main.py \
        ...
        --cache_dir="$RECSYS26_ROOT/datasets/diffgrm_cache_paper" \
        --log_dir="$run_dir" \
        --ckpt_dir="$run_dir/ckpt" \
...
src = Path("${RECSYS26_ROOT}/third_party/RQ_VAE_Recommender/configs/rqvae_setrec_smoke.gin")
dst = Path(run_dir) / f"rqvae_setrec_{domain}.gin"

text = src.read_text()
text = text.replace('train.dataset_split="beauty"', f'train.dataset_split="{domain}"')
text = text.replace(
    'train.save_dir_root="/data/xqp_data/RecSys26/logs/rqvae_recommender_setrec/rqvae/"',
