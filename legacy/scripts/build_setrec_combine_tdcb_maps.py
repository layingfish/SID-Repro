...
def load_microlens_titles(path: Path) -> dict[int, str]:
    ...
    if len(lower) >= 2 and lower[0] in {"item", "item_id", "video_id"}:
        ...
        # MicroLens-1M_title.csv has no header and contains unbalanced quotes in
        # title text, so use physical-line parsing instead of csv.reader.
        with path.open("r", encoding="utf-8", newline="") as f:
            for line in f:
                line = line.rstrip("\r\n")
                if "," not in line:
...
def main() -> None:
    ...
    ap.add_argument(
        "--microlens_titles",
        type=Path,
        default=Path("/data/xqp_data/RecSys26/datasets/microlens_50k/raw/MicroLens-50k_titles.csv"),
    )
    ap.add_argument(
    ...
    ap.add_argument(
        "--amazon_meta",
        type=Path,
        default=Path("/data/xqp_data/RecSys26/datasets/amazon23_vg/raw/meta_Video_Games.jsonl.gz"),
    )
    ap.add_argument(
        "--yelp_business",
        type=Path,
        default=Path(
            "/data/xqp_data/RecSys26/datasets/yelp_open/raw/extracted/yelp_academic_dataset_business.json"
        ),
    )
