...
RUN_ROOT="${LOG_ROOT}/${ID_DIR}/${MODEL_SIZE}/$(date +%Y%m%d_%H%M%S)__gpu${GPU}"
...
case "${MODEL_SIZE}" in
  ...
  M03-t5l)
    ...
    ;;
  *)
    echo "Unknown or disabled model_size for clean20k diagnostic: ${MODEL_SIZE}" >&2
    exit 2
    ;;
esac
...
echo "[cell] protocol=microlens1m_retrain_clean20k"
...
echo "[cell] gin=${GIN}"
echo "[cell] hf=${HF}"
echo "[cell] train_iterations=20000 save_model_every=5000 early_stop_patience=3"
