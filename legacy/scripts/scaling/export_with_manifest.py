#!/usr/bin/env python3
"""用 CachedIdTokenizer 替换 SemanticIdTokenizer，复用 test_only_tiger.py 导出。

用法:
    python export_with_manifest.py \
        ...
        --decoder_ckpt logs/scaling_ml50k/I02-semrq/M01-t5s/.../checkpoint_xxx.pt \
        --domain microlens_50k \
        --export_path logs/scaling_ml50k/I02-semrq/M01-t5s/.../pred_topk.jsonl \
        [--beam_size 20 --topk_items 10]

原理同 train_with_manifest.py：monkey-patch SemanticIdTokenizer。
"""
...
def main():
    ...
    # 从 checkpoint 推断 num_user_tokens
    import torch
    ...
    del ckpt, state
    # HF tokenizer add_special_tokens 后 resize_token_embeddings 可能做 padding
    # 需要反推出训练时传入的 num_user_tokens 参数，使 export 重建的 vocab 精确匹配 ckpt
    # build_t5_tokenizer_and_model 的流程：
    #   1. tokenizer = AutoTokenizer (vocab=32100)
    #   2. add sid_tokens + uid_tokens as special_tokens → tokenizer vocab 增加
    #   3. model.resize_token_embeddings(len(tokenizer)) → 可能 round up
    # 所以我们需要试不同的 num_user_tokens 直到 resize 后 vocab == ckpt_vocab
    # 简单方法：从 1 开始尝试
    ...
    if args.warm_only_export:
        ...
        if warm_items_path is None:
            setrec_warm_items_path = Path("/data/xqp_data/RecSys26/third_party/SETRec/data") / args.domain / "warm_item.npy"
        ...
        data_dir_candidates = [
            Path("/data/xqp_data/RecSys26/third_party/SETRec/data") / args.domain,
    ...
    # 构造 test_only_tiger.py 的 sys.argv
    sys.argv = [
        "test_only_tiger.py",
        "--domain", args.domain,
        "--backbone", "t5_small",
        "--rqvae_ckpt", "UNUSED",  # MockTokenizer 不需要这个
