...
echo "[cell] protocol=microlens1m_vgstyle"
echo "[cell] id=${ID_DIR} compat=${COMPAT_DIR} model=${MODEL_SIZE} align_model=${ALIGN_MODEL_SIZE} gpu=${GPU}"
echo "[cell] run_root=${RUN_ROOT}"
echo "[cell] compat_root=${COMPAT_ROOT}"
echo "[cell] hf=${HF}"
echo "[cell] export_batch_size=${EXPORT_BATCH_SIZE} export_scope=warm_only"
...
CUDA_VISIBLE_DEVICES="${GPU}" "${PY}" "${ROOT}/scripts/scaling/train_with_manifest.py" \
  ...
  --vg_main_table_align_model_size "${ALIGN_MODEL_SIZE}" \
  --gin_override \
